...
The developer-ready Client API endpoints can be accessed via one of two types of Bearer tokens: [OAuth Access Tokens](/api-info/client/authentication/oauth) issued by an SSO provider or [Glean-issued tokens](/api-info/client/authentication/glean-issued). Both methods can be used concurrently, but we recommend using OAuth access tokens for a more secure authentication for Client API requests, scoped to the individual user it's issued to.
