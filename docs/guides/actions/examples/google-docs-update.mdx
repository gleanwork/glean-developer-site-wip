...
<Steps>
  <div>
    <h3>Step 1: Action Manifest Configuration</h3>
  ...
  <div>
    <h3>Step 2: API Specification Development</h3>
...
paths:
  ...
  <div>
    <h3>Step 3: Authentication Setup</h3>
...
Scopes: https://www.googleapis.com/auth/documents`}</code></pre>
  ...
  <div>
    <h3>Step 4: Testing and Deployment</h3>
...
<CardGroup cols={2}>
  ...
  <Card title="Efficient Operations" icon="Gauge">
...
<CardGroup cols={2}>
  <Card title="Document Operations" icon="FilePen">
...
<Card title="Comprehensive Testing" icon="TestTube">
...
If you encounter issues during setup or execution, follow these diagnostic steps:

<Steps>
  <div>
    <h3>Step 1: Verify OAuth Configuration</h3>
    <ul>
      <li>Check OAuth credentials are correctly configured</li>
      <li>Ensure the Google Docs API is enabled</li>
      <li>Validate that the redirect URI exactly matches your action's configuration</li>
    </ul>
  </div>
  ...
  <div>
    ...
    <ul>
      <li>Verify the user has appropriate permissions in both Glean and Google Docs</li>
      <li>Confirm document access permissions</li>
      <li>Check API quota and rate limits</li>
    </ul>
  </div>

  <div>
    <h3>Step 3: Debug Implementation</h3>
    <ul>
      <li>Validate document ID extraction and formatting</li>
      <li>Test with different document types and sizes</li>
      <li>Monitor API response codes and error messages</li>
    </ul>
  </div>
</Steps>
...
<Card title="Ongoing Operations" icon="ChartLine">
...
- Plan for Google Docs API version updates
