...
In order to filter search results to show documents that match certain fields via the [/search REST API](https://developers.glean.com/client/operation/search/), you need to construct a list of `facetFilter` objects and pass it into the [`requestOptions`](https://developers.glean.com/client/operation/search/#tag/Search/operation/search!path=requestOptions&t=request) object.
