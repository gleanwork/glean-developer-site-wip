...
function calculateTokenOverlap(tokens1: Array<string>, tokens2: Array<string>): number {
...
function findBestMatch(oldPath: string, newPaths: Array<string>): string | null {
  let bestMatch = null;
  let bestScore = 0;
  const threshold = 0.3;
  ...
  console.log(`${oldPath} -> ${bestMatch || 'NO MATCH'} (score: ${bestScore.toFixed(3)})`);
...
async function generateRedirects(): Promise<void> {
  try {
    ...
    console.log(`📊 Found ${oldPaths.length} old paths and ${newPaths.length} new paths`);
