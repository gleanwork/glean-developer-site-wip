...
const RequestSchemaComponent: React.FC<Props> = ({ title, body, style }) => {
  ...
  const renderProperty = (
    ...
    return (
      <div key={name} className={styles.propertyContainer}>
          ...
          <div className={styles.enumContainer}>
            ...
            <div className={styles.enumValues}>
              {schema.enum.map((value: string) => (
                <span key={value} className={styles.enumValue}>
                  {value}
                </span>
              ))}
            </div>
        ...
        {schema.example && (
          <div className={styles.example}>Example: {schema.example}</div>
        )}
  ...
  const hasProperties =
    ...
    'properties' in firstBody &&
    firstBody.properties;
  const requiredArray =
    firstBody &&
    typeof firstBody === 'object' &&
    'required' in firstBody &&
    Array.isArray(firstBody.required)
      ? firstBody.required
      : [];
